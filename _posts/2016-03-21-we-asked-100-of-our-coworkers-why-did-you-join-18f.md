...
minds in design and technology.” - {{ "shawn" | team_link }}</blockquote>
</div>
...
was the only thing that felt worthwhile to me.” - {{ "maya" | team_link}}</blockquote></div>
...
NPR.” - {{ "brethauer" | team_link }}</blockquote></div>
...
I'm staying because I have found my people.” - {{ "jeremy" | team_link}}</blockquote></div>
...
government.” - {{ "erica" | team_link }}</blockquote></div>
...
editorial strategy, working in the open, and working remotely.” -{{ "nicole-fenton" | team_link }}</blockquote></div>
...
there.” - {{ "egoodman" | team_link }}</blockquote></div>
...
government needs to be stressful and difficult.” - {{ "mhz" | team_link }}</blockquote></div>
...
enriched my life more than I could imagine.” - {{ "gail" | team_link }}</blockquote></div>
...
engaged citizen.” - {{ "jthibault" | team_link }}</blockquote></div>
...
for meaningful work.” - {{ "christopher-goranson" | team_link }}</blockquote></div>
...
lose less sleep.” - {{ "robin-carnahan" | team_link }}</blockquote></div>
...
spectacular team GSA was building.” - {{ "alla" | team_link }}</blockquote></div>
...
only thing that ever has.'” - {{ "mchopson" | team_link }}</blockquote></div>
...
{{ "jessie-posilkin" | team_link }}</blockquote></div>
...
future as we were when we made one giant leap for mankind.” - {{ "vdavez" | team_link }}</blockquote></div>
...
making government more accessible to individuals.” - {{ "joshbailes" | team_link }}</blockquote></div>
...
people and learn completely new skills” - {{ "matt" | team_link }}</blockquote></div>
...
“I joined 18F to help people connect to their government.” - {{ "boone" | team_link }}</blockquote></div>
...
working on some of the problems I’d been identifying.” - {{ "andre" | team_link }}</blockquote></div>
...
{{ "melody" | team_link }}</blockquote></div>
...
thing.” - {{ "sarah" | team_link}}</blockquote></div>
...
{{ "larry-bafundo" | team_link }}</blockquote></div>
...
even when I didn’t know 18F was it.” - {{ "leah" | team_link }}</blockquote></div>
...
and really scale out the digital efforts in government.” - {{ "gray" | team_link }}</blockquote></div>
...
“I saw 18F as the best place to magnify my ability to drive positive change in the world.” - {{ "bret" | team_link }}</blockquote></div>
...
environment that melded those areas into one!” - {{ "anna" | team_link}}</blockquote></div>

<div class="pquote">
{{ "raphy" | team_photo }}
<blockquote>

“I joined 18F because of the people and the mission.” - {{ "raphy" | team_link }}</blockquote></div>
...
gentlemen, is my 'why'.” - {{ "alan-atlas" | team_link }}</blockquote></div>
...
government and the American people is incredibly high.” - {{ "kane" | team_link }}</blockquote></div>
...
of it.” - {{ "angela-colter" | team_link }}</blockquote></div>
...
“I joined 18F to help government modernize and streamline their IT processes and mostly to make a difference.” - {{ "mike-hsu" | team_link }}</blockquote></div>
...
culture, policy, and technology facets.” - {{ "jayhuie" | team_link }}</blockquote></div>
...
platform that serves sailors around the world.” - {{ "michael-cata" | team_link }}</blockquote></div>
...
and people.” - {{ "romke" | team_link }}</blockquote></div>
...
today.” - {{ "kedelman" | team_link }}</blockquote></div>
...
nurturing a thoughtful, inclusive, and gratifying culture.” - {{ "ed-mullen" | team_link }}</blockquote></div>
...
missions.” - {{ "noah" | team_link }}</blockquote></div>
...
interesting challenges.” - {{ "kimberdowsett" | team_link }}</blockquote></div>
...
interactions delightful. #AspirationalFTW To paraphrase <a href="https://www.youtube.com/watch?v=gvAIvauhQGQ&feature=youtu.be&t=30m3s">President Obama's speech in Selma last year</a>: It's such an honor to be able to work on improving this great nation of ours.” - {{ "kara" | team_link }}</blockquote></div>
...
to help our government realize its mission.” – {{ "holly" | team_link }}</blockquote></div>
...
problems worked on were for a greater purpose.” - {{ "lenny-bogdonoff" | team_link }}</blockquote></div>
...
change government forever, for the better.” - {{ "nick" | team_link }}</blockquote></div>
...
and ultimately the American people!” - {{ "christine" | team_link }}</blockquote></div>
...
Act implementation.” {{ "kaitlin" | team_link}}</blockquote></div>
...
huge draw.” - {{ "afeld" | team_link }}</blockquote></div>
...
impact on a lot of people's lives.” - {{ "annalee" | team_link }}</blockquote></div>
...
how to fix.” - {{ "yoz" | team_link }}</blockquote></div>
...
from.” - {{ "jacobharris" | team_link }}</blockquote></div>
...
{{ "jeremiak" | team_link }}</blockquote></div>
...
efficiency, transparency, and agility. Our impact is mighty.” - {{ "cm" | team_link}}</blockquote></div>
...
activism and non-profit community work.” - {{ "sasha" | team_link }}</blockquote></div>
...
responsive, and effective government from inside.” - {{ "eric" | team_link }}</blockquote></div>
...
team of amazing people.” - {{ "amanda-robinson" | team_link }}</blockquote></div>
...
18F and started the hiring process.” - {{ "marco" | team_link }}</blockquote></div>
...
better place.” - {{ "micahsaul" | team_link }}</blockquote></div>
...
from diverse backgrounds.” – {{ "becky" | team_link }}</blockquote></div>

<div class="pquote">
{{ "clinton-troxel" | team_photo }}
<blockquote>

“I joined 18F to work for people instead of for money.” – {{ "clinton-troxel" | team_link }}</blockquote></div>
...
I am able to help as part of that within.” - {{ "jessie" | team_link }}</blockquote></div>
...
are building <a href="https://beta.fec.gov/">beta.fec.gov</a>.” {{ "lindsay" | team_link }}</blockquote></div>
...
digital tools and transparency.” - {{ "cristina" | team_link }}</blockquote></div>
...
of joining 18F.” {{ "amanda-schonfeld" | team_link }}</blockquote></div>
...
my dream coworkers.” - {{ "will" | team_link }}</blockquote></div>
