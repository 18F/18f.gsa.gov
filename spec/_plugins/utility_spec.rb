...
RSpec.describe Jekyll::Utility do
  before(:each) do
    @utility_class = Class.new.new # instantiate a nothing class
    @utility_class.extend(Jekyll::Utility)

    @nav_items = YAML.safe_load_file(File.join(Dir.pwd, "spec/_data/navigation.yml"))["assigned"]

    @nav_item_join = YAML.safe_load_file(File.join(Dir.pwd, "spec/_data/join.yml"))

    blog_variations = YAML.safe_load_file(File.join(Dir.pwd, "spec/_data/blog.yml"))
    ...
    project_variations = YAML.safe_load_file(File.join(Dir.pwd, "spec/_data/s_projects.yml"))
    ...
    project_variations = YAML.safe_load_file(File.join(Dir.pwd, "spec/_data/p_projects.yml"))
    @nav_item_project = project_variations["project"]
    @nav_item_project_with_children = project_variations["project_with_children"]
    @nav_item_project_child = @nav_item_project_with_children["children"][0]

    @first_post = YAML.safe_load_file(File.join(Dir.pwd, "spec/_posts/page.rb"))
    @project_page = YAML.safe_load_file(File.join(Dir.pwd, "spec/_posts/project_page.rb"))
    @post_url = @first_post["url"]
    @post_url = @utility_class.clip_char(@post_url.to_s.downcase, "/")

    @services_wieght = YAML.safe_load_file(File.join(Dir.pwd, "spec/_data/serv_weight.yml"))

    @products_wieght = YAML.safe_load_file(File.join(Dir.pwd, "spec/_data/prod_weight.yml"))
  end
