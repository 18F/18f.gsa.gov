...
You can still make changes and keep building and improving your system after its initial ATO. [Further changes and functionality may flow through Agile and DevOps processes (or through a Change Control Board, in a waterfall environment)]({{ "/2021/03/02/using-agile-and-devops-to-get-better-results-than-a-change-control-board/" | url }}). You may need to seek a new or modified ATO if or when you find that your system will need to change in major ways — for example, if you need a new system architecture or need to connect to a new external system.
